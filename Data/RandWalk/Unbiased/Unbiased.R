# Simuation of individual based model with no bias.
...
for(k in 1:(time_m-1)){
  ...
  for(l in mv.agents){
    # Generate direction moved:
    mv.theta = runif(1, min = 0, max = 2*pi)
...
dist_file = data.frame(distance = as.vector(t(sqrt((X.mat[,,1])^2+(X.mat[,,2])^2))),time = rep(times_unique,each=N))
